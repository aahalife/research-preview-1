...
struct RumiMarkView: View {
    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: 1.0 / 20.0, paused: !animated || reduceMotion || scenePhase != .active)) { timeline in
            ...
            ZStack {
                Circle().fill(Theme.warm.opacity(0.07))
                Circle().fill(Theme.warm.opacity(0.12)).padding(size * 0.10)
                Circle().fill(Theme.warm.opacity(0.20)).padding(size * 0.20)
                Circle().fill(Theme.warm.opacity(0.32)).padding(size * 0.29)
                Circle().fill(Theme.warm).padding(size * 0.37)
            }
