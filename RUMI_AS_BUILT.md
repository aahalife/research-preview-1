...
The supplied `chat-portal-internals.pdf` defines the auth sequence, not a complete current wire schema. New native `RumiTokenProvider`, `RumiSocketConnection`, `RumiWireProtocol` and `RumiWebSocketTransport` implement a **provisional text compatibility profile** with an authenticated host broker, in-memory short-lived JWT, Bearer `getConfig`, delegate-confirmed WSS opening, auth/text/control frames, message-ID filtering, matching completion and bounded refresh on the same socket. Tenant API keys never enter the client. No token is put in URLs, logs or snapshots; redirects and insecure endpoints fail closed. There is no fallback to Rork after a backend failure, automatic replay, invented tool receipt or clinical capability toggle. See the [current connection handoff](RUMI_RESCOPE.md#switchable-ai-connection--september-17) for exact assumptions and deployment prerequisites.

Backend URLs/current host login and server traces are not yet provided. Therefore actual interoperability, authenticated patient ownership, VPC routing, external tool safety, true server cancellation/deduplication and native WebSocket voice remain **unverified/unimplemented**, not completed. The text adapter opens/closes per operation using its workspace ID; current continuous-session and stop/edit behavior still need contract alignment. Temporary AI still uses `anthropic/claude-sonnet-4.6`. This change routes existing contextual and review-first capabilities; it does not complete every retained feature. Web and Functions are unchanged in this continuation; screenshot production remains paused.

Validation: native simulator build passed. The focused selection `NudgeTests` plus the AI-mode switching UI test passed **39 tests in 59 seconds**. The first run passed 36 unit tests but the UI check encountered duplicate accessibility nodes for the same alert button; the test selector was corrected, not waived. Coverage includes broker credential isolation, token expiry, frame order/refresh, stale-message filtering, incomplete/auth-failed replies, no fallback for chat or prep, saved mode/draft/context isolation, late-output suppression, cancellation before text transmission and voice/text operation ownership. Final native simulator build also passed. Broadened `NudgeTests` plus four relevant UI checks passed **42 tests in 86 seconds**; the artifact/link/QMS audit passed with all accepted presentation assets unchanged. Current-server interoperability remains unverified.
...
- `CareContext` provides selected-item identity, scenario, source, dated content where supplied and editable question. Medication/lab/record/insight/Currents/Life and symptom entry use it. Context is inspectable before Send and persists with the unsent composer and sent turns. Entering AI no longer auto-sends. Untouched suggested questions follow item changes; replacing an edited contextual draft requires an explicit choice. Full details use a scrolling sheet rather than overflowing the composer.
- `WorkflowReviewView` shows the basis, prepared work, intended recipient, exact editable draft and reviewed-not-sent status. Record conflicts and symptom notes use it instead of local sent-success flags. Medication detail offers contextual AI help and a direct non-AI request draft. Messages exposes `WorkflowLibraryView` for saved work. Context/source lookup reopens edited drafts despite new view UUIDs; autosaves are debounced and flushed on close/background.
- Today now surfaces only actual saved prepared work, not seeded delivery suggestions or fabricated active-task counts. Illustrative task ideas remain accessible under the agent network's example disclosure.
- Agent approval, including the older task-idea cards, no longer completes a task or appends a fabricated external outcome. It creates/reopens a draft. Paused domain examples cannot open a new review. Declining an already saved task keeps the task identity so its draft remains recoverable. The network's first screen explains prepared work; seeded domain/task activity is behind Explore agent examples and explicitly illustrative.
- Removed unsupported claims of pharmacy-ready refills, one-tap delivery, blanket interaction checking and live connection account status on affected screens. Cost-support examples now have a medication entry point and truthful unavailable enrollment; no discount is applied. Legacy reports/bills/requests and broader fixtures still need their separate completion work.
- Marcus's generic record-category fixtures/sources no longer appear in other scenarios. Those category views now honestly show no available source entries; the other personas' own medication/series workspaces remain. This is not full patient identity isolation or source reconciliation.
- The screen library maps all 36 attachment features and seven additions, with contextual AI roles and explicit unbuilt capabilities. Ten new records/contextual-help slides were accepted. The visit screenshot chapter was cancelled, so further generation is paused and full feature coverage is incomplete. The manifest retains twenty accepted slides and ten staged, unaccepted visit originals separately. Earlier accepted slides were recovered after the capture tool replaced the active output set. No App Store upload was performed.
- Records screenshots precede the final source-details-sheet and draft-replacement safeguards. They are dated captures, not proof of a final-build exhaustive visual check. VoiceOver/Dynamic Type/device testing, clinical evaluation and external-service evidence remain outstanding.

### Extension validation

Final `runChecks(ios)` succeeded. Final test selection `NudgeTests` plus `NudgeUITests/NudgeUITests`: **31 passed in 128 seconds**. Earlier selections in this extension passed 23 and 28 tests before the additional safeguards were added. Coverage includes selected context in transport, no sending on entry, draft replacement consent, snapshot round-trip, source-based reopening, pause/decline recovery, no fabricated task outcomes and the medication-to-chat UI return path. A mutable-record-collection compile error was fixed during the scenario-boundary correction.

The standalone asset/link audit validates twenty accepted original/slide pairs, local library links, all 43 feature rows and the unchanged QMS note. Some original capture files are JPEG bytes stored under the screenshot tool's `.png` names; accepted presentation slides are actual PNGs. No new screenshots were generated after cancellation. This is simulator/source/artifact evidence, not production clinical or service validation.
