...
- Fasten test public/private keys, webhook signing secret, registered redirect, authorized tenant and production data-processing agreements are not yet available/verified. Local Demo is not vendor sandbox, and neither is production. The official FooClinic sandbox uses synthetic identities; never send actual demographics from Demo.
...
- Next agent: start here, inspect `RUMI_AS_BUILT.md` current-delivery section, then the unchecked approved-plan stages. Do not restart design or present local Fasten import as a vendor integration. Securely request `FASTEN_TEST_PUBLIC_ID` and `FASTEN_TEST_PRIVATE_KEY` first (server-only environment storage), then configure the registered redirect and signed webhook endpoint. The signing secret is requested after endpoint setup, not invented. Request the user's WebSocket event/auth contract before implementing that adapter.
